...
from sklearn.metrics import mean_squared_error, roc_auc_score, average_precision_score, f1_score, log_loss, precision_score,recall_score
...
class DBTA:
    ...
    def test_(self, data_generator, model, repurposing_mode = False, test = False, test_result = None):
        ...
        if self.binary:
            ...
            return roc_auc_score(y_label, y_pred), average_precision_score(y_label, y_pred), recall_score(y_label, outputs), precision_score(y_label, outputs),  f1_score(y_label,outputs), log_loss(y_label, y_pred), y_pred, getROCE(outputs, y_label, 0.5), getROCE(outputs, y_label, 1), getROCE(outputs, y_label, 2), getROCE(outputs, y_label, 5)
        else:
            if repurposing_mode:
                return y_pred
    ...
    def train(self, train, val = None, test = None, verbose = True, test_result=None):
        ...
        if self.binary:
            valid_metric_header.extend(["AUROC", "AUPRC", "RECALL","PRC","ROCE0","ROCE1","ROCE2","ROCE5"])
        else:
        ...
        for epo in range(train_epoch):
            ...
            if val is not None:
                ...
                with torch.set_grad_enabled(False):
                    if self.binary:  
                        ## binary: ROC-AUC, PR-AUC, RECALL, cross-entropy loss
                        auc, auprc, RECALL,precision,f1, loss, logits,  ROCE0, ROCE1, ROCE2, ROCE5 = self.test_(validation_generator, self.model, test_result = test_result)
                        lst = ["epoch " + str(epo)] + list(map(float2str,[auc, auprc, RECALL, precision, ROCE0, ROCE1, ROCE2, ROCE5]))
                        ...
                        if verbose:
                            print('Validation at Epoch '+ str(epo + 1) + ', AUROC: ' + str(auc)[:7] + \
                              ' , AUPRC: ' + str(auprc)[:7]  + ' , RECALL: '+str(RECALL)[:7] + ' , precision: '+str(precision)[:7]  + ' , ROCE0: '+str(ROCE0)[:7]  + ' , ROCE1: '+str(ROCE1)[:7] + ' , ROCE2: '+str(ROCE2)[:7] + ' , ROCE5: '+str(ROCE5)[:7]+ ' , Cross-entropy Loss: ' + \
                              str(loss)[:7])
                    else:  
        ...
        if test is not None:
            ...
            if self.binary:
                auc, auprc, RECALL,precision,f1, loss, logits,  ROCE0, ROCE1, ROCE2, ROCE5 = self.test_(testing_generator, model_max, test = True, test_result = test_result)
                test_table = PrettyTable(["AUROC", "AUPRC", "RECALL","PRC","ROCE0","ROCE1","ROCE2","ROCE5"])
                test_table.add_row(list(map(float2str, [auc, auprc, RECALL, precision, ROCE0, ROCE1, ROCE2, ROCE5])))
                if verbose:
                    print('Validation at Epoch '+ str(epo + 1) + ' , AUROC: ' + str(auc)[:7] + \
                      ' , AUPRC: ' + str(auprc)[:7] + ' , RECALL: '+str(RECALL)[:7] + ' , ROCE0: '+str(ROCE0)[:7]  + ' , ROCE1: '+str(ROCE1)[:7] + ' , ROCE2: '+str(ROCE2)[:7] + ' , ROCE5: '+str(ROCE5)[:7]+ ' , Cross-entropy Loss: ' + \
                      str(loss)[:7])				
            else:
    ...
    def predict(self, df_data):
        ...
        auc, auprc, RECALL,precision,f1, loss, logits,  ROCE0, ROCE1, ROCE2, ROCE5 = self.test_(generator, self.model)
        return auc, auprc, RECALL,precision,f1, loss, logits,  ROCE0, ROCE1, ROCE2, ROCE5
