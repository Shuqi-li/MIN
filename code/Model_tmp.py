...
class Model(nn.Module):
    def __init__(self, config, device):
        ...
        if self.config.interaction:
            ...
            self.MLP_NoInter = MLP(self.config.transformer_emb_size_target  + self.config.transformer_emb_size_drug, 1, self.config.MLP_hidden_size_list_N)
    ...
    def forward(self, data):
        ...
        if self.config.interaction:
            if self.config.use_score:
                ...
                emb, msa_emb = self.emb(e, msa)
                #target encoder
                target_enc_1d =  self.Transformer_target_local(target_1d)
            ...
            if self.config.target_2d_encoder:
                ...
                if self.config.use_score:
                    ...
                    #target encoder
                    target_enc =  self.Transformer_target_local(target_1d)
                out_put = torch.cat([drug_enc, target_enc], dim=-1)
